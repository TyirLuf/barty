...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($id)) {
        ...
        if ($resultFuncionario->num_rows > 0) {
            ...
            // Verificar se nenhuma função foi selecionada
            if (empty($funcaoId)) {
                echo "Escolha uma função!";
                // Aqui você pode adicionar o código para exibir a mensagem de erro, redirecionar ou tratar o erro de alguma forma
                // Por exemplo, você pode redirecionar de volta ao formulário de edição com uma mensagem de erro específica:
                // header("Location: ./editar_func.php?id=$id&error=Escolha uma função!");
                // exit();
            } else {
