...
function cadastrarUsuario($nome, $senha, $confirmar_senha, $email, $tipo)
{
    ...
    $insert = "INSERT INTO clientes(`primeiro_nome`, `Senha`, `Email`, Status) VALUES (?,?,?,?,?)";
    $q = $pdo->prepare($insert);
    $q->execute(array(ucwords($nome), $senha, $email, 1));
    Banco::desconectar();
    return "Sucesso!";
}
...
function validarExistencia($email)
{
    $pdo = Banco::conectar();
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $consulta = "SELECT cliente.id as cliente_id,  FROM clientes WHERE EMAIL = '$email'";
    Banco::desconectar();
